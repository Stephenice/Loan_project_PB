...
function loan_calculator(range, rangeYear, interestRate) {
  ...
  const discount_factor =
    ((1 + interestRate_r) ** monthly - 1) /
    (interestRate_r * (1 + interestRate_r) ** monthly);
  const payment_monthly = (borrowAmount / discount_factor).toFixed(2);
  const payment_monthly_format = payment_monthly
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  ...
  const totalRepayable_format = totalRepayable_calcu
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  ...
  const totalInterest_format = totalInterest_calcu
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
